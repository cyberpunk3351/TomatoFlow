...
mkdir -p "tomato-flow.AppDir"
cp -r ../appimage_deb/data/usr "tomato-flow.AppDir"

cd "tomato-flow.AppDir"
mkdir -p "usr/bin"
mkdir -p "usr/lib"
...
# We need AppRun to be installed as tomato-flow.AppDir/AppRun.
...
cp "usr/share/icons/hicolor/256x256@2/apps/tomato-flow.png" .DirIcon
ln -sf "usr/share/icons/hicolor/256x256@2/apps/tomato-flow.png" "tomato-flow.png"

ln -sf "usr/share/applications/tomato-flow.desktop" "tomato-flow.desktop"
...
( cd "/root/.cache/tauri" && wget -q -4 -N https://raw.githubusercontent.com/tauri-apps/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh )
...
OUTPUT="tomato-flow_1.0.0_amd64.AppImage" "/root/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage" --appimage-extract-and-run --appdir "tomato-flow.AppDir" --plugin gtk ${gst_plugin} --output appimage
